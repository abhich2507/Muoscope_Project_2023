...
Aerog->AddMaterial(CO2, fractionmassCO2);
 ...
 G4ThreeVector pos4 = G4ThreeVector(0*cm,0*cm, 10*cm);
  G4Box * solidShape4  = new G4Box("Shape4", (45/2)*cm, (45/2)*cm, (5/2)*cm);
...
*/
  ...
  //////////////////////////////////////////////////////////////////////////////////////////////
   ...
   G4Box* blockSolid = new G4Box("BlockSolid", 2/2.0, 2 / 2.0, 2 / 2.0);

    // Create a logical volume for the block
    G4LogicalVolume* blockLogical = new G4LogicalVolume(blockSolid, Uranium, "BlockLogical");

    // Set visualization attributes for the block (you can customize the color)
    G4VisAttributes* visAttributes = new G4VisAttributes(G4Color(0.0, 1.0, 0.0, 0.3)); // Green with transparency
    blockLogical->SetVisAttributes(visAttributes);

    // Create a physical volume for the block and place it in the mother volume
     //new G4PVPlacement(0, pos4, blockLogical, "Block", logicEnv, false, 0, checkOverlaps);

      // Define the position for the letter "L"
    G4ThreeVector startPos = G4ThreeVector(-10.0 * cm, 0.0, 10.0 * cm);

    // Define the voxel positions for the letter "L"
    std::vector<G4ThreeVector> voxelPos = {
        G4ThreeVector(0, 0, 0), G4ThreeVector(0, 1, 0), G4ThreeVector(0, 2, 0),
        G4ThreeVector(1, 2, 0)
    }
    
      G4double voxelSize = 1.0 * cm;
    G4double halfVoxelSize = 0.5 * voxelSize;

for (const auto& voxelPos : voxels)
    {
        G4ThreeVector voxelPosition = startPos + G4ThreeVector(voxelPos[0]* voxelSize, voxelPos[1]() * voxelSize, voxelPos[3]() * voxelSize);
        new G4PVPlacement(0, voxelPosition, voxelLogical, "Voxel", logicEnv, false, 0, true);
    }
