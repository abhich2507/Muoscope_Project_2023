...
G4Element* elSi = new G4Element("Silicon", "Si", z=14., a= 28.0855*g/mole);
  double density = 2.64 *g/cm3;
    G4Material* Quartz = new G4Material("Quartz", density, int nel= 2);
     Quartz-> AddElement(elSi, int natoms=1);
 ...
 fLogicPlaneG2 = new G4LogicalVolume*[fNbOfPlanes] ;
  ...
  for (G4int copyNo=0; copyNo<fNbOfPlanes; copyNo++) {
    if(copyNo<2){ 
       ...
       G1_pos=  G10 + (3*cm)*copyNo;
       G2_pos=  G20 + (3*cm)*copyNo;
       }

    if(copyNo>=2){ 
        z_pos=  z1 + (3*cm)*copyNo;
        G1_pos=  G11 + (3*cm)*copyNo;
        G2_pos=  G12 + (3*cm)*copyNo;
        }
