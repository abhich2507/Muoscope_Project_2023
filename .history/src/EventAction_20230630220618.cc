 ...
 void EventAction::EndOfEventAction(const G4Event* event)
{
  ...
  for (G4int iHit=0; iHit<nHits; ++iHit){
   ...
   G4float tracklength_test= tracklength*pow(10,-15);
    ...
    analysisManager->AddNtupleRow(0);
      ...
      myrootManager->ROOTTreeStruct.ScintHitStation[myrootManager->ROOTTreeStruct.NScintHit]=  PlaneNb;
      ...
      myrootManager->ROOTTreeStruct.ScintHitTrackLength[myrootManager->ROOTTreeStruct.NScintHit]= (G4float) tracklength_test;
       G4cout<<"nhitO = "<<iHit<<G4endl;
      G4cout<<"nhit = "<<nScintHit<<G4endl;
  ...
  if ( ( printModulo == 0 ) || ( eventID % printModulo != 0 ) ) 
	...
	auto  primary= event->GetPrimaryVertex(i)->GetPrimary(q);
      G4ThreeVector pz = primary->GetMomentum();
