...
vector<float> HE[4]= {HE0,HE1,HE2,HE3};
 ...
 TH1F* histAngle = new TH1F("histAngle","scattering angle",200,-1.5,1.5);
 ...
 char nameZ[15], titleZ[20];
   ...
   for (Int_t i = 0; i < 4; i++) {
     ...
     TCanvas *c8 = new TCanvas("c8","Histograms related Planes posX");
...
TF1* fitFunc = new TF1("fitFunc", MyFitFunction, 1.5, 3.2,2);
      Double_t max = histGenTheta->GetMaximum();
      std::cout<<max<<endl;
     fitFunc->SetParameters(0,30);
     fitFunc->SetParameters(1,2);
     c8->cd();
    histGenTheta->Draw();
