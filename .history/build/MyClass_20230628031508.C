...
Point findClosestPoint(const Point &p1, const Point &v1, const Point &p2, const Point &v2)
{
   ...
   closestPoint.x = p1.x + t1 * dir1.x;
   closestPoint.y = p1.y + t1 * dir1.y;
   closestPoint.z = p1.z + t1 * dir1.z;
...
canvas1->Divide(1,2);
TH3F* histogram1 = new TH3F("histogram1", "2D Histogram", 200, -250, 250, 200, -250, 250,200,35,165);
   ...
   for (int q = 0; q < Sx[1].size(); q++)
   {
      ...
      cout<<poca.x<<endl;
      histogram1->Fill(poca.x, poca.y,poca.z);
                         }
   }
