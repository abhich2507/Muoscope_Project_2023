...
Double_t MyFitFunction(Double_t* x, Double_t* par) {
       Double_t value =par[0]*TMath::Power(TMath::Cos(x), par[1]);  // cos^2(x)
...
TF1* fitFunc = new TF1("fitFunc", MyFitFunction, -2, 3,2);
      Double_t max = histGenTheta->GetMaximum();
      std::cout<<max<<endl;
     fitFunc->SetParameter(0,8000);
     fitFunc->SetParameter(1,2.);
     c8->cd();
    fitFunc->Draw();
    //histGenTheta->Fit("fitFunc","","",0.01,1.4);
