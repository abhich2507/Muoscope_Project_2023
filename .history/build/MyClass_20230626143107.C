...
vector<float> HE[4]= {HE0,HE1,HE2,HE3};
 ...
 TH1F* histAngle = new TH1F("histAngle","scattering angle",100,-1,200);
...
for(int q =0; q<=4390; q++){ 
 Vector v1(Sx[1][q]-Sx[0][q],Sy[1][q]-Sy[0][q],Sz[1][q]-Sz[0][q]);
 Vector v2(Sx[3][q]-Sx[2][q],Sy[3][q]-Sy[2][q],Sz[3][q]-Sz[2][q]);
   double angle = calculateAngle(v1, v2);
   double angleDegrees = angle * 180.0 / M_PI;
   
   histAngle->Fill(angleDegrees);
cout << "The angle between vA and vB is: " << angleDegrees << " degrees." << std::endl;
...
for(int j =0; j<4; j++){ 
   for(int i=0; i<=Sz[j].size(); i++){
      cout<<"Zavg = "<<Sz[j][i]<<endl;
      cout<<"Yavg = "<<Sy[j][i]<<endl;
      cout<<"Xavg = "<<Sx[j][i]<<endl;
