...
Point findClosestPoint(const Point &p1, const Point &v1, const Point &p2, const Point &v2)
{
   Point closestPoint1;
   Point closestPoint2;
   ...
   Point dir1 = {v1.x - p1.x, v1.y - p1.y, v1.z - p1.z};
   Point dir2 = {v2.x - p2.x, v2.y - p2.y, v2.z - p2.z};
   ...
   closestPoint1.x = p1.x + t1 * dir1.x;
   closestPoint1.y = p1.y + t1 * dir1.y;
   closestPoint1.z = p1.z + t1 * dir1.z;

   closestPoint2.x = p2.x + t2 * dir2.x;
   closestPoint2.y = p2.y + t2 * dir2.y;
   closestPoint2.z = p2.z + t2 * dir2.z;

   Point closestPont.x = (closestPoint1.x-closestPoint2.x)/2.
   Point closestPont.y = (closestPoint1.y-closestPoint2.y)/2.
   Point closestPont.z = (closestPoint1.z-closestPoint2.z)/2.
...
canvas1->Divide(1,2);
TH2F* histogram1 = new TH2F("histogram1", "2D Histogram", 200, -250, 250, 200, -250, 250);
   ...
   for (int q = 0; q < Sx[1].size(); q++)
   {
      ...
      Point p1 = {Sx[0][q], Sy[0][q], Sz[0][q]};
      Point V1 ={Sx[1][q] - Sx[0][q], Sy[1][q] - Sy[0][q], Sz[1][q] - Sz[0][q]};
      Point p2 ={Sx[2][q], Sy[2][q], Sz[2][q]};
      Point V2={Sx[3][q]-Sx[2][q], Sy[3][q] - Sy[2][q], Sz[3][q] - Sz[2][q]};
      Point poca= findClosestPoint(p1,V1,p2,V2);
      cout<<poca.x<<endl;
      histogram1->Fill(poca.x, poca.y);
                         }
   }
